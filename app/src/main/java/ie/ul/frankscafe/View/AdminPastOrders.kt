...
class AdminPastOrders(val application: Application, mainFragmentManager: MainFragmentManager) : Fragment(R.layout.adminpastorders) {
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        val view: View = inflater!!.inflate(R.layout.pastorders, container, false)

        var orderList = PastOrderViewModel(application).getAllPastOrders()
        var orderListView = view?.findViewById<ListView>(R.id.PastOrders)
        val arrayAdapter: ArrayAdapter<String> =
            ArrayAdapter(application, android.R.layout.simple_list_item_1, orderList.map { it.OrderContents + " " + it.OrderTime.substringBefore('T')} )
